...
public class GlobalMembersStringalign
{
	public static void stringalign(byte[] ain, byte[] bin, Doub mispen, Doub gappen, Doub skwpen, byte[] aout, byte[] bout, byte[] summary)
	{
		Int i = new Int();
		Int j = new Int();
		Int k = new Int();
		...
		Int ia = ain.length();
		Int ib = bin.length();
		MatDoub cost = new MatDoub(ia+1,ib+1);
		cost[0][0] = 0.;
		for (i = 1;i<=ia;i++)
